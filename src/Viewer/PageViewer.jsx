...
import { serializedComponentsReplacer, serializedComponentsReviver } from '../utils/serialize';
...
export default function PageViewer(props) {
    ...
    if (coreWorker.current) {
      coreWorker.current.onmessage = function (e) {
        ...
        if (e.data.messageType === "updateRenderers") {
          ...
          console.log(`We should send toast message: ${e.data.args.message}`);
  ...
  function resetPage({ changedOnDevice, newCid, newAttemptNumber }) {
    ...
    if (newAttemptNumber !== attemptNumber) {
      console.error(`Reverted activity as attempt number changed on other device`);
      ...
      console.error(`Reverted page to state saved on device ${changedOnDevice}`);
  ...
  function startCore() {
    ...
    coreWorker.current = new Worker(new URL('../Core/CoreWorker.js', import.meta.url), {
      type: 'module'
    })
