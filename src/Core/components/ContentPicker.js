            ...
            definition: function ({ dependencyValues }) {
                ...
                for (let [
                    ...
                    if (dependencyValues.separateByTopic) {
                        let topicsForChild = dependencyValues[
                            `childTopics${ind}`
                        ...
                        if (topicsForChild.length > 0) {
                            for (let topic of topicsForChild) {
                                ...
                                let topicWithCase =
                                    allLowercaseTopics[topicLower];
                                if (!topicWithCase) {
                                    topicWithCase = allLowercaseTopics[
                                        topicLower
                                    ] = topic;
                                }

                                let childrenWithTopic =
                                    childrenByTopic[topicWithCase];
                                if (!childrenWithTopic) {
                                    childrenWithTopic = childrenByTopic[
                                        topicWithCase
                                    ] = [];
                                }
                            ...
                            let defaultTopic =
                                dependencyValues.defaultTopicLabel;
                            let childrenWithoutTopic =
                                childrenByTopic[defaultTopic];
                            if (!childrenWithoutTopic) {
                                childrenWithoutTopic = childrenByTopic[
                                    defaultTopic
                                ] = [];
                            }
                ...
                return {
                    setValue: {
                        childIndicesToRender:
                            dependencyValues.selectedIndices.map((x) => x - 1),
                    },
                };
            },
