...
Today that place holds the predefined styles and any style definitions you write. The predefined styles come in named, coordinated sets called **style palettes** — an author can swap the whole set with a single [`<stylePalette>{:dn}`](../reference/stylePalette), and every distinction encoded in style numbers survives the swap, because the distinctions were never tied to particular colors in the first place. And Doenet lets **readers** remap those definitions — including switching the whole document to a palette that works for them, such as the pure-luminance `grayscale` palette: a platform hosting Doenet content can offer its readers controls to pick colors (and other settings) that *they* can best tell apart, with every style-1 object shifting together and every style-2 object shifting together. A reader's remapping wins over everything the author wrote — that is the point of the indirection. The distinctions you encoded survive, re-expressed in a form that works for that reader. (Doenet already checks the contrast of each style definition — and of every built-in palette — for this reason.)
