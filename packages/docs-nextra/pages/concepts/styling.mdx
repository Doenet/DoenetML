...
Today that place holds the predefined styles and any style definitions you write. The predefined styles come in named, coordinated sets called **style palettes** — an author can swap the whole set with a single [`<stylePalette>{:dn}`](../reference/stylePalette), and every distinction encoded in style numbers survives the swap, because the distinctions were never tied to particular colors in the first place. As Doenet evolves, it will also let **readers** remap those definitions — so a reader can pick colors (and other settings) that *they* can best tell apart, with every style-1 object shifting together and every style-2 object shifting together. The distinctions you encoded survive, re-expressed in a form that works for that reader. (Doenet already checks the contrast of each style definition — and of every built-in palette — for this reason.)
