...
<ComponentDisplay name='mathInput'/>

`<mathInput/>{:dn}` is an [Input](../concepts/essentialConcepts#component-types)
...
```doenet-editor-horiz
<p>
  <mathInput name="userExpression">
    <label>Type <c>x^2 + sqrt4 - 1/y</c></label>
  </mathInput>
</p>
...
```doenet-editor-horiz
<p>
  <mathInput name="userAngle">
    <label>Enter <c>pi/4</c></label>
  </mathInput>
</p>
<p>
  ...
  <math>2/10</math> = 
    <answer symbolicEquality="true">
      <mathInput name="userFraction" prefill=" / ">
        <shortDescription>simplified fraction</shortDescription>
      </mathInput>
      <award>1/5</award>
    </answer>
...
### Example: `<mathInput/>{:dn}` on a `<graph>{:dn}`


```doenet-editor-horiz
<p>
  <mathInput name="userAnchor" prefill="(0,0)">
    <label>Where should we put the <tage>mathInput</tage>?</label>
  </mathInput>
</p>

<graph size="small">
  <shortDescription>A regular pentagon with a draggable math input.</shortDescription>
  <mathInput name="expr" anchor="$userAnchor" prefill="x^2">
    <shortDescription>math input anchored at $userAnchor</shortDescription>
  </mathInput>
...
A `<mathInput/>{:dn}` can be placed on a `<graph>{:dn}`. Use the `anchor` attribute to adjust its starting location. Click inside the field to edit it, and grab its label (or the grip shown when it has no label) to drag it around.
...
<p>
  <mathInput name="userInput" prefill="x^() sin()">
    <shortDescription>fill in the blanks</shortDescription>
  </mathInput>
</p>
...
if used, are rendered as gray input boxes. 
...
<p>
  <mathInput name="userInput" prefillLatex="\binom{n}{ }">
    <shortDescription>fill in the bottom of the binomial coefficient</shortDescription>
  </mathInput>
</p>
...
<p>
  <mathInput name="fraction1" prefill="\frac{ }{ }" format="latex">
    <label>Enter a fraction (LaTeX prefill)</label>
  </mathInput>
</p>

<p>
  <mathInput name="fraction2" prefill=" / ">
    <label>Enter a fraction (default prefill)</label>
  </mathInput>
</p>
...
```doenet-editor-horiz
<p>
  <mathInput name="function1">
    <label>Type <m>h(x) = x^2</m> (without <attr>functionSymbols</attr>)</label>
  </mathInput>
</p>
<p>You've entered $function1</p>

<p>
  <mathInput name="function2" functionSymbols="h">
    <label>Type <m>h(x) = x^2</m> (with <attr>functionSymbols</attr>)</label>
  </mathInput>
</p>
...
```doenet-example
<p>
  <mathInput name='mi1' prefill="xyz">
    <label>With <attr>splitSymbols="true"</attr>, by default</label>
  </mathInput>
</p>
<p>The square of $mi1 is <math simplify>$mi1^2</math>.</p>
      -------------------------
<p>
  <mathInput splitSymbols="false" name="mi2" prefill="xyz">
    <label>With <attr>splitSymbols="false"</attr></label>
  </mathInput>
</p>
...
```doenet-editor-horiz
<p>Enter two numbers that add to 5.
...
<graph size="small"><shortDescription>A graph of a circle through three points</shortDescription>
...
<p>
  <mathInput bindValueTo="$circ.radius">
    <label>Bound radius of the circle through the points $A, $B, and $C</label>
  </mathInput>
</p>

<p>Alternate way to access the bound radius:
  <mathInput>
    <shortDescription>radius reference (read-only display)</shortDescription>
    $circ.radius
  </mathInput>
</p>
```
...
component as a child of the `<mathInput>{:dn}`.
...
```doenet-example
<p>
  <mathInput name="union" unionFromU>
    <label>Type <m>A U B</m> (with spaces between)</label>
  </mathInput>
</p>
<p>Value interpreted as $union.</p>

<p>
  <mathInput name="noUnion">
    <label>Type <m>A U B</m> (with spaces between)</label>
  </mathInput>
</p>
...
<p>
  <mathInput name="mi1">
    <shortDescription>numeric input with hideNaN default</shortDescription>
    <number />
  </mathInput>
  = $mi1
</p>
<p>
  <mathInput name="mi2" hideNaN="false">
    <shortDescription>numeric input with hideNaN false</shortDescription>
    <number />
  </mathInput>
  = $mi2
</p>
...
<p>
  <mathInput name="mi2" removeStrings="%">
    <label>Enter <m>20\%</m> (with <c>%</c> removed)</label>
  </mathInput>
  = $mi2
</p>
...
<p>
  <mathInput name="mi1">
    <label>Enter <m>ax^2 + bx + c</m> (default width)</label>
  </mathInput>
  = $mi1
</p>

<p>
  <mathInput name="mi2" minWidth="100">
    <label>Enter <m>ax^2 + bx + c</m> (minWidth 100)</label>
  </mathInput>
  = $mi2
</p>

<p>
  <mathInput name="mi3" minWidth="20">
    <label>Enter <m>ax</m> (minWidth 20)</label>
  </mathInput>
  = $mi3
</p>
```
...
<graph size="small">
  <shortDescription>A circle with two math inputs, one fixed and one draggable.</shortDescription>
  <mathInput
    draggable="false"
    prefill="cannot drag"
    anchor="(-3,-5)"
  >
    <shortDescription>fixed math box</shortDescription>
  </mathInput>
  <mathInput prefill="drag me" anchor="(3,5)">
    <shortDescription>draggable math box</shortDescription>
  </mathInput>
...
The `draggable` attribute can be set to false to prevent users from moving the `<mathInput/>{:dn}` on a `<graph>{:dn}`.
...
<mathInput name="mi1" prefill="3/4 x">
  <shortDescription>example mathInput with prefill</shortDescription>
</mathInput>
...
<mathInput 
  ...
  <shortDescription>example mathInput with prefillLatex</shortDescription>
</mathInput>
...
```doenet-example
<mathInput name="mi1">
  <shortDescription>any math expression</shortDescription>
</mathInput>
= $mi1
...
<p>
  <mathInput name="mi1">
    <label>Enter an expression with text and math</label>
  </mathInput>
</p>
...
<p>
  <mathInput name="mi1">
    <label>Enter an expression such as <c>sqrt(2)/3</c></label>
  </mathInput>
</p>

<p><c>latex</c> = $mi1.latex</p>
<p><c>mi1</c> = $mi1</p> 
```



The `latex` property of a `<mathInput/>{:dn}` stores its content as LaTeX code, 
updated when the reader presses Enter or leaves the input.
...
<p>
  <mathInput name="mi1">
    <label>Type an expression involving latex, or using a mathematical operator</label>
  </mathInput>
</p>
