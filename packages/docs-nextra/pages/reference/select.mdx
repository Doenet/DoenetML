...
<ComponentDisplay name='select'/>

`<select>{:dn}` is a [General Operator](../concepts/essentialConcepts#component-types)
...
For fine-grained control over which option appears in which named variant
of the document, see the `selectForVariants` attribute on
[`<option>{:dn}`](option) — it lets you pin specific options to specific
variants instead of relying on random selection.
...
<graph><shortDescription>A graph of two functions and their y-intercepts</shortDescription>
  $f[1] $f[2]
