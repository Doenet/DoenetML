...
The following examples illustrate use of the `<function>{:dn}` tag attributes (Part 1).
...
<graph size="small">
  <shortDescription>graph of <m>f</m> on its domain</shortDescription>
  $f
</graph>
...
```doenet-editor-horiz
<graph size="small">
  <shortDescription>three interpolated functions with specified minima</shortDescription>
  <function 
    name="f" 
    minima="(4,5) (7,-3)" 
  />
