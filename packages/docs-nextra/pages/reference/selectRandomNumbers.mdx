...
so `<selectRandomNumbers/>{:dn}` has no `resample` action. For values a reader can redraw, 
use [`<sampleRandomNumbers/>{:dn}`](sampleRandomNumbers) instead.
...
The `logNormal` type raises $e$ to the power of a normal variable, so every value 
...
```doenet-editor-horiz
<p>Ten exam scores from a class with two distinct groups:
  <selectRandomNumbers 
    name="x" 
    type="normalMixture" 
    means="55 85" 
...
Whether the values come out looking bimodal depends on the components. Two 
equally weighted components of equal spread separate into two visible humps only 
once their centers are more than $2\sigma$ apart.
