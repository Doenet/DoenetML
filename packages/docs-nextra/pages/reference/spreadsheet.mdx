...
Add `header` to a `<row>{:dn}` and its cells are drawn in bold. A header row can still be edited
like any other. The emphasis is not only visual: the cells of a header row are announced to a
screen reader as headers for the columns beneath them.
