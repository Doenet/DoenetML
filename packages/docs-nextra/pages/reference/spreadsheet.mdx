...
Add `header` to a `<row>{:dn}` and its cells are drawn in bold, against the same shading the
spreadsheet uses for its `A`, `B`, `C` and `1`, `2`, `3` labels. This is emphasis only: a header
row can still be edited like any other. Marking up the text inside a cell has no effect, since a
cell contributes only its text to the spreadsheet — `header` is how a row is set apart.
