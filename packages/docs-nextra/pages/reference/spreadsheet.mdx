...
<ComponentDisplay name='spreadsheet'/>

`<spreadsheet>{:dn}` is a [General Operator](../concepts/essentialConcepts#component-types) 
...
### Example: build `<spreadsheet>{:dn}` with columns
...
### Example: emphasizing a header row


```doenet-editor-horiz
<spreadsheet minNumRows="4" minNumColumns="3">
  <row header>
    <cell>Name</cell>
    <cell>Type</cell>
    <cell>Exp</cell>
...
Add `header` to a `<row>{:dn}` and its cells are drawn in bold. This is emphasis only: a header
row can still be edited like any other, and it keeps the ordinary cell background so that it is
not read as part of the spreadsheet's own `A`, `B`, `C` and `1`, `2`, `3` labels. Marking up the
text inside a cell has no effect, since a cell contributes only its text to the spreadsheet —
`header` is how a row is set apart. It is also what a screen reader is told: the cells of a header
row are announced as headers for the columns below them, so the emphasis is not only visual.



---

### Example: cells the user cannot edit
...
<spreadsheet minNumRows="4" minNumColumns="4">
  ...
  <row>
    ...
    <cell>2</cell>
    <cell>10</cell>
    <cell fixed>= C2/B2</cell>
  </row>
  <row>
    <cell fixed>2</cell>
    <cell>4</cell>
    <cell>18</cell>
    <cell fixed>= C3/B3</cell>
  </row>
</spreadsheet>
```
...
Add `fixed` to a `<cell>{:dn}` and the user cannot change it: the cell is greyed and clicking it
opens no editor, though it can still be selected and copied. Use it to protect labels and computed
results while leaving the data cells open. Adding `fixed` to a `<row>{:dn}` fixes every cell of
that row, and adding it to the `<spreadsheet>{:dn}` itself makes the whole grid read-only.
