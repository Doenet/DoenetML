...
When `colorInputsSeparately` is set and the answer uses `numAwardsCredited` with separate
`<award>{:dn}`s for different parts of the response, each input box is colored based on the
awards that reference it rather than all inputs receiving the same overall credit color.
In the example above, the numerator and denominator boxes color independently — green when
their respective award is satisfied, red when not.

This works by detecting which `<when>{:dn}` conditions reference each input.
It works best when each input is covered by **at most one positive-credit award** per
"tier". When multiple simultaneously-satisfiable awards cover the same input, only the
largest awarded `creditAchieved` is used to determine that input's color — so an input
may show partial or full credit even if not all of its covering awards were satisfied.
