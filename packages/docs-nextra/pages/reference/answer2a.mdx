...
The following examples illustrate use of the `<answer>{:dn}` tag attributes (Part 1).
...
```doenet-example
  <p>Answer this to activate the next problem: 
    <answer name="ans1" symbolicEquality><label><m>2 - 1 = </m></label>1</answer></p>
  <p>Next problem: 
    <answer name="ans2" disabled="$ans1.creditAchieved != 1" symbolicEquality><label><m>$ans1.submittedResponse + 1 = </m></label>2</answer></p>
```
...
```doenet-example
  <document name="assignment">
    ...
    <p>
    <answer weight="2" name="ans1"><label><m>1 + 1 = </m></label>2</answer></p>

    <p>Question 2 (less important)</p>
    <p>
    <answer name="ans2"><label><m>1 + 2 = </m></label>3</answer></p>
...
<p>The complementary color for red is 
  <answer inline>
    <shortDescription>complementary color for red</shortDescription>
    <choice>blue</choice>
...
<p>
  <answer>
    <label>What is the complementary color for purple?</label>
    <choice>blue</choice>
    <choice>green</choice>
...
<p>
  <answer type="text" handGraded expanded>
    <label>In one sentence, describe the theme of the story</label>
  </answer>
</p>
```
...
<p>
  <answer matchPartial>
    <mathInput name="userNumber"><label>Enter an even number greater than 6</label></mathInput>
    <award>
      <when>$userNumber > 6 and <mod>$userNumber 2</mod> = 0</when>
    </award>
  </answer></p>

<p><answer matchPartial><label>What is <m>(1,2,3)+(4,5,6)</m>?</label>(5,7,9)</answer></p>
<p>
  <answer matchPartial selectMultiple>
    <label>Select all the warm colors</label>
    <choice credit="1">yellow</choice>
    <choice>green</choice>
...
<p>
  <answer symbolicEquality><label><m>2x + 5x = </m></label>7x</answer>
</p>
<p>
  <answer symbolicEquality><label><m>x^2 + 2x^2 + 3x + x</m></label>3x^2 + 4x</answer>
</p> 
```
...
```doenet-example
<p>
  <answer><label><m>2x + 5x = </m></label>7x</answer>
</p>
<p>
  <answer forceFullCheckWorkButton><label><m>2x + 5x</m></label>7x</answer>
</p>  
<p>
  <answer forceFullCheckWorkButton submitLabel="check!"><label><m>2x + 5x</m></label>7x</answer>
</p>
...
<graph size="small">
  <shortDescription>graph for moving a point</shortDescription>
  <point name="P" />
</graph>
```
...
<p>Enter two numbers that add up to 5:
  ...
  <answer name="ans1">
    <shortDescription>two numbers that add up to 5</shortDescription>
...
<p>Enter two numbers that multiply to 12:
  ...
  <answer name="ans2" forceSmallCheckWorkButton>
    <award><when>$m1 $m2=12</when></award>
  </answer>
</p>
...
```doenet-editor-horiz
<p><label for="$ans">Enter anything mathematically equivalent to <m>x^2+2x+1</m></label>:
  <answer name="ans">x^2+2x+1</answer></p>

<p><label for="$ans">Enter <m>x^2+2x+1</m> exactly</label>:
  <answer name="ans" symbolicEquality>x^2+2x+1</answer></p>

<p><label for="$ans">Enter <m>x^2+2x+1</m> or some mathematically equivalent forms (not factored)</label>:
  <answer name="ans" symbolicEquality simplifyOnCompare>x^2+2x+1</answer></p>

<p>
  <choiceInput name="simpOnComp" inline preselectChoice="1">
    <label>Pick level of simplification</label>
    <choice>none</choice>
...
<p>
  <answer symbolicEquality simplifyOnCompare="$simpOnComp"><label>Enter <m>x^2+2x+1</m></label>x^2+2x+1</answer></p>
```
...
<p><label for="$ans">Enter <m>x^2+2x+1</m> in factored or unfactored form</label>:
  <answer name="ans" symbolicEquality simplifyOnCompare expandOnCompare>x^2+2x+1</answer></p>
```
...
<p>
  <answer>
    <label>Enter <m>1,2,3,4,5</m></label>
    <mathList>1 2 3 4 5</mathList>
  </answer></p>

<p>
  <answer unorderedCompare>
    <label>Now enter <m>1,2,3,4,5</m>, but scramble the order up</label>
    <mathList>1 2 3 4 5</mathList>
  </answer></p>

<p>
  <answer>
    <label>Again, enter <m>1,2,3,4,5</m>, but scramble the order up</label>
...
<answer matchPartial matchByExactPositions>
  <label>Earn partial credit by getting some of the components of <m>(1,2,3,4)</m> correct</label>
  (1,2,3,4)
</answer>

<p><answer matchPartial>
  <label>Earn partial credit by getting some of the numbers <m>1,2,3,4</m> entered in order</label>
  1,2,3,4
</answer></p>
```
...
### Attribute Example: colorInputsSeparately

```doenet-editor-horiz
<p>Enter a fraction equal to <m>\dfrac{2}{3}</m>:</p>
<answer name="ans" numAwardsCredited="2" colorInputsSeparately>
  <fractionInput name="fi">
    <label>Enter a fraction equal to two thirds</label>
  </fractionInput>
  <award credit="0.5"><when>$fi.numerator = 2</when></award>
  <award credit="0.5"><when>$fi.denominator = 3</when></award>
</answer>
```
When `colorInputsSeparately` is set and the `<award>{:dn}`s of the answer are structured appropriately,
then each input will be colored based on the correctness of its value
(green if correct, red if incorrect, and orange if partially correct).

At present, `colorInputsSeparately` requires a particular setup of the `<award>{:dn}`s so that the credit associated
with each input can be correctly calculated.
It works best when:
1. `numAwardsCredited` is set to the number of inputs used,
2. each input is referenced in its own `<award>{:dn}`, and
3. the credits for those awards add up to 1.

(Additional `<award>{:dn}`s can be added to create partial-credit scenarios for each input.)

Using other patterns for awards,
such as referencing multiple inputs in an award
or having multiple awards credited for a single input,
may lead to unexpected results.

A common use case for `colorInputsSeparately` is to color the numerator and denominator
boxes of a fraction input separately, as illustrated above.
The attribute also applies to multiple `<mathInput>{:dn}`s connected to an answer via
`forAnswer`:

```doenet-editor-horiz
<mathInput name="mi1" forAnswer="$ans2">
  <label>Enter a value greater than 0</label>
</mathInput>
<mathInput name="mi2" forAnswer="$ans2">
  <label>Enter a value greater than 5</label>
</mathInput>
<answer name="ans2" numAwardsCredited="2" colorInputsSeparately>
  <award credit="0.5"><when>$mi1 > 0</when></award>
  <award credit="0.5"><when>$mi2 > 5</when></award>
</answer>
```

Each `<mathInput>{:dn}` box colors independently: green when its award is satisfied,
red when not.
