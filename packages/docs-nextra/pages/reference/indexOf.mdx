import { DoenetViewer, DoenetEditor, DoenetExample } from "../../components"

import { ComponentDisplay, AttrPropDisplay } from "../../components"


# `<indexOf>{:dn}`

<ComponentDisplay name='indexOf'/>

`<indexOf>{:dn}` is a [General Operator](../concepts/essentialConcepts#component-types)
component that renders, for each value given by the `target` attribute, the position of the
first of its arguments equal to that value. Indices start at 1, and a target that does not
occur in the list gives 0. To find where a value *belongs* in a sorted list rather than
whether it is there at all, use [`<searchSorted>{:dn}`](searchSorted).

<AttrPropDisplay name='indexOf'/>

## Examples

### Example: finding a number


```doenet-editor-horiz
<numberList name="nl">10 20 30 20</numberList>

<p>Values: $nl</p>
<p>First position of 20: <indexOf target="20">$nl</indexOf></p>
<p>Position of 25: <indexOf target="25">$nl</indexOf></p>
```



The value 20 occurs twice and the first occurrence is reported. A value that is absent
gives 0, which is a position no list entry has.



---

### Example: finding a name


```doenet-editor-horiz
<textList name="names">Ann Bob Cal Dee</textList>

<p>Names: $names</p>
<p>Cal is in position <indexOf type="text" target="Cal">$names</indexOf>.</p>
<p>Eve is in position <indexOf type="text" target="Eve">$names</indexOf>.</p>
```



`type="text"` tells `<indexOf>{:dn}` to read the target as text rather than as a
number.



---

### Example: looking up several targets at once


```doenet-editor-horiz
<textList name="names">Ann Bob Cal Dee</textList>
<textList name="wanted">Cal Ann Eve</textList>

<p>Roster: $names</p>
<p>Looking for: $wanted</p>
<p>Positions: <indexOf type="text" target="$wanted">$names</indexOf></p>
```



There is one result per target, in the order the targets were given. Eve is not on the
roster, so her slot is 0 while the others keep their positions.



---

### Example: looking up a parallel list


```doenet-editor-horiz
<setup>
  <textList name="symbols">H He Li Be</textList>
  <numberList name="masses">1.008 4.003 6.94 9.012</numberList>
</setup>

<indexOf name="which" type="text" target="Li">$symbols</indexOf>

<p>The atomic mass of Li is $masses[$which].</p>
```



Finding a position in one list and reading it out of another is the usual reason to
want `<indexOf>{:dn}` rather than a test for membership.


---

### Example: indexing without naming the lookup


```doenet-editor-horiz
<setup>
  <textList name="symbols">H He Li Be</textList>
  <numberList name="masses">1.008 4.003 6.94 9.012</numberList>
</setup>

<p>The atomic mass of Li is $masses[<indexOf type="text" target="Li">$symbols</indexOf>].</p>
```



The `<indexOf>{:dn}` can be written between the index brackets instead of being named
and referenced. Both find the same position, so which to use is mostly a matter of how
the sentence reads: naming it keeps a long index out of the prose, and is what to do
when the same position is wanted more than once.

One difference worth knowing: a reference cannot carry on past an index written this
way. `$masses[<indexOf …/>]` is fine because nothing follows the bracket, but to reach
a property after the index — `$points[$i].x`, say — name the position first and
reference it.





---

## Attribute Examples

### Attribute Example: target


```doenet-editor-horiz
<textList name="cities">
  <text>Boston</text><text>New York</text><text>Chicago</text>
</textList>

<text name="wanted">New York</text>

<p>Written out: <indexOf type="text" target="New York">$cities</indexOf></p>
<p>Referenced: <indexOf type="text" target="$wanted">$cities</indexOf></p>
```



`target` is read the way bare children are: whitespace separates one target from the
next, so `target="New York"` asks for two of them and neither is on the list. A value
with a space in it is written as a `<text>{:dn}` of its own and referenced — which is
also how the list holding it is built, since `<textList>Boston New York</textList>{:dn}`
is three cities and not two.
