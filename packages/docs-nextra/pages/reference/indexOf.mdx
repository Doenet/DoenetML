...
Because 0 is not the index of any item, the reason for one is always reported. A target
that is simply absent is an info message — a list still settling can legitimately not
contain it yet — while omitting `target` altogether is a warning, since nothing about
that document could ever produce an answer.
