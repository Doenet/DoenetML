...
<ComponentDisplay name='callAction'/>

`<callAction>{:dn}` is a [General Operator](../concepts/essentialConcepts#component-types)
component that triggers an action based on a click interaction or a specified boolean condition. The 
available actions are:
- `addChildren`: Adds one or more children to the target component. Supported on
  `<graph>{:dn}`, `<stickyGroup>{:dn}`, and the sectioning components (such as
  `<section>{:dn}`, `<subsection>{:dn}`, `<paragraphs>{:dn}`, `<problem>{:dn}`,
  `<exercise>{:dn}`, `<example>{:dn}`, and `<aside>{:dn}`).
- `deleteChildren`: Deletes one or more children from the target component. Supported on the
  same components as `addChildren`.
...
<graph name="g" size="small">
  <shortDescription>An empty graph to which points can be added</shortDescription>
</graph>
```
...
<graph name="g" size="small">
  <shortDescription>An empty graph to which points can be added or deleted</shortDescription>
</graph>
```
...
<graph size="small">
  <shortDescription>A graph of a point at random coordinates</shortDescription>
  <point>($x, $y)</point>
</graph>
```
...
```doenet-example
<graph name="g" size="small">
  <shortDescription>A graph with a callAction button to add a point</shortDescription>
...
```doenet-editor-horiz
<graph name="g">
  <shortDescription>Graph with a button to add a circle and point</shortDescription>
...
<graph grid size="small">
  <shortDescription>A graph of a line with a draggable point and a circle that submits the answer when clicked</shortDescription>
  <point name="p1" fixed/>
...
```doenet-editor-horiz
<answer name="ans">
  <label>What mythological creature can only be befriended by gentle maidens?</label>
...
<div name="exampleDoenetML">
  <graph size="tiny">
    <shortDescription>An empty graph</shortDescription>
  </graph>
  <slider/>
  <answer>
...
<graph name="g" size="small">
  <shortDescription>A graph with a draggable point that triggers adding another point</shortDescription>
...
<booleanInput name="bool"><label>Trigger a rectangle</label></booleanInput>
<callAction 
  triggerWith="$bool" 
  actionName="addChildren" 
  target="$g" >
...
<p>Trigger a line:</p>
<p><answer name="ans"><label>Type <m>x</m></label>x</answer></p>
<callAction 
  triggerWith="$ans" 
  actionName="addChildren" 
  target="$g" >
...
<graph name="g" size="small">
  <shortDescription>A graph with a clickable point that triggers adding a circle</shortDescription>
