...
`<cobwebPolyline>{:dn}` renders an interactive cobweb diagram for visualizing iteration of a one-variable function
$x_{n+1} = f(x_n)$. When placed inside a [`<graph>{:dn}`](graph), the user clicks to add successive vertices
