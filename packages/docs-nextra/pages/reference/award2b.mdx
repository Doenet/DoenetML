...
<p>
  <answer>
    <label>Enter a number within about <m>1\%</m> of <m>100</m></label>
...
<p>
  <answer>
    <label>Enter a number within 0.01 of <m>100</m></label>
...
<p>
  <answer>
    <label>Enter an expression close to <m>\sin(\pi x)</m></label>
...
<p>
  <answer>
    <label>Enter a number within <m>1</m> of <m>100</m></label>
...
<p>
  <answer>
    <label>Enter a number within <m>10</m> of <m>100</m></label>
...
```doenet-example
  <p>
    <answer>
      <label>Allow a 1% error in <m>2x^3+4</m>, but only in the coefficient 2 and constant 4</label>
      <award allowedErrorInNumbers="0.01" symbolicEquality>2x^3+4</award>
    </answer>
  </p>
  <p>
    <answer>
      <label>Allow a 1% error in <m>2x^3+4</m>, in any of the 3 numbers</label>
      <award allowedErrorInNumbers="0.01" symbolicEquality includeErrorInNumberExponents>2x^3+4</award>
    </answer>
  </p>
```
...
<p>
  <answer type="text">
    <label>Enter tTkLp with any capitalization</label>
...
<p>
  <answer>
    <label>Either <m>x^2</m> or <m>X^2</m> is acceptable</label>
...
```doenet-example
    <p>
      <answer name="ans">
        <label>Enter <m>f()</m> with no arguments.</label>
        <award matchBlanks>f()</award>
      </answer>
    </p>
```
...
### Attribute Example: feedbackCodes
```doenet-example
<p>
  <answer name="ans">
    <label>Simplify <m>\frac{-8}{-16}</m></label>
    <mathInput name="mi" />
    <award>1/2</award>
    <award credit="0.7" feedbackCodes="oneSignError">-1/2</award>
  </answer>
</p>
...
The `feedbackCodes` attribute selects feedback by *code* rather than by
literal string. Codes resolve through any
[`<feedbackDefinition>{:dn}`](feedbackDefinition) in scope and fall back to
the four [built-in codes](feedbackDefinition#built-in-feedback-codes)
(`goodJob`, `numericalError`, `oneSignError`, `twoSignErrors`). The matched
text is delivered through the answer's `feedbacks` property, so the
`$ans.feedbacks` reference is what actually renders it.
