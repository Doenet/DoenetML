...
<graph>
  <function copySource="ode.numericalSolution" name="f"/>
...
```doenet-editor-horiz
<p>A ball weighing <mathInput name="wt" prefill="128"/> is suspended from a spring. The spring, with constant <m>k = </m><mathInput name="k" prefill="64"/> <m>\frac{\text{lb}}{\text{ft}}</m> stretches <mathInput name="s1" prefill="2"/> feet from its initial, unweighted length. The ball is then placed in motion by compressing the spring <mathInput name="s2" prefill="-0.5" /> feet <em>upwards</em> from equilibrium, and then released.</p>
...
<odesystem name="pendulum" hideInitialCondition variables="theta omega" initialConditions="$ic_t_theta.y 1" maxIterations="10000">
  ...
  <function copySource="pendulum.numericalSolution[1]" name="thetaFn"/>
  <function copySource="pendulum.numericalSolution[2]" name="omegaFn" />
...
<function name="f" copySource="ode.numericalSolution[1]" displayDecimals="8" hide/>
...
<p>Accessing numerical solutions, method 2:</p>
  ...
  <function name="position" copySource="ode.numericalSolution[1]"/>
  <function name="velocity" copySource="ode.numericalSolution[2]"/>
...
the $i^{th}$ function, or define new functions referencing the `numericalSolution[i]` property with the `copySource` attribute.
