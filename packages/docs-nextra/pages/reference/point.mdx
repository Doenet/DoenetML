...
By default, a point's label is drawn with an opaque background so it stays legible over an axis or grid line; `maskLabel="false"` (shown on point `B`) restores a transparent label background. See the [`<label>{:dn}`](label) reference for a side-by-side comparison.
