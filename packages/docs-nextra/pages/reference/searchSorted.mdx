...
Write `sort` on the operator and it puts the values in order itself, so a list that is not
already sorted can be searched directly.
...
ordering to the operator, and `side` goes on meaning what it means.
