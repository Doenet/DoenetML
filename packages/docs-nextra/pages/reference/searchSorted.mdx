...
<p>The same position in one step: <searchSorted sort target="80">$scores</searchSorted></p>
```
...
position again. The last line gets there directly: `sort` hands the ordering to the
...
Prefer `sort` when the ordering is wanted only for the search. Keep a separate
