...
and denominator boxes are colored independently as long as the numerator and denominator
are referenced in separate `<award>{:dn}`s.
Each box turns green when its
