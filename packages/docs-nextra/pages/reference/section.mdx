...
<section collapsible startOpen><title>Open by default</title>
  <p>This section starts expanded because <c>startOpen</c> is set.</p>
</section>
```
...
closed; use `startOpen` to have it start expanded.

(`<aside>{:dn}` and `<proof>{:dn}` are collapsible and closed by default without
