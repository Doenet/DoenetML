...
```doenet-editor-horiz
<p><mathInput name="userN"><label>Enter a number other than 0</label></mathInput></p>
...
```doenet-editor-horiz
<graph size="small">
  <shortDescription>A graph displaying a number at (5,5)</shortDescription>
...
<graph size="small">
  <shortDescription>A graph displaying two numbers, one draggable and one not</shortDescription>
...
The `draggable` attribute determines whether the `<number>{:dn}` can be 
