...
### Example: `<point>{:dn}` constrained to a `<point>{:dn}`
...
Points can take `constraints{:dn}` as children. Several different types of constraints are possible. In this case, the `constrainTo{:dn}` constraint references a named `<point>{:dn}`.
