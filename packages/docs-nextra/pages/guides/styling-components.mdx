...
<graph size="small" xmin="-1" xmax="8" ymin="0" ymax="9" identicalAxisScales>
  <shortDescription>Eight points and eight filled circles, one pair in each of the first eight styles of the categorical palette</shortDescription>
  ...
  <circle center="(6,8)" radius="0.4" filled styleNumber="1" />
  <circle center="(6,7)" radius="0.4" filled styleNumber="2" />
  <circle center="(6,6)" radius="0.4" filled styleNumber="3" />
  <circle center="(6,5)" radius="0.4" filled styleNumber="4" />
  <circle center="(6,4)" radius="0.4" filled styleNumber="5" />
  <circle center="(6,3)" radius="0.4" filled styleNumber="6" />
  <circle center="(6,2)" radius="0.4" filled styleNumber="7" />
  <circle center="(6,1)" radius="0.4" filled styleNumber="8" />
</graph>
```

The points show each style's marker color and shape; the filled circles beside them add each style's line color (the outline) and fill color (the interior), which a palette also sets. Every built-in palette meets WCAG contrast requirements in both light and dark mode, and palettes vary marker shapes and line widths alongside colors, so the distinctions your style numbers encode never rest on color alone. If your audience includes colorblind readers, choose one of the colorblind-friendly palettes: `okabeIto` (based on the Okabe–Ito palette) is the usual first choice, `tolHighContrast` maximizes distinction (even in grayscale) with a compact set of four styles, and `tolBright`, `tolMuted`, and `ibm` offer more looks with the same design goal. For readers with total colorblindness, the `grayscale` palette maximizes pure luminance separation. See the [`<stylePalette>{:dn}` reference](../reference/stylePalette) for the full list.
