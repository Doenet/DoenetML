...
<ComponentDisplay name='choiceInput'/>

`<choiceInput>{:dn}` is an [Input](../concepts/essentialConcepts#component-types) 
...
```doenet-example
<choiceInput>
  <label>I agree with the statement.</label>
  <choice>Agree</choice>
...
A `<choiceInput>{:dn}` is used to collect user responses that do not require
validation. A nested `<label>{:dn}` child supplies the prompt for the choices.
...
<answer>
  <choiceInput>
    <label>Pick the correct statement</label>
...
<graph size="small"
 ><shortDescription>A graph of a $menu</shortDescription>
...
<choiceInput selectMultiple>
  <label>Pick the colors you like</label>
  <choice>Red</choice>
...
<answer>
  <choiceInput selectMultiple matchPartial>
    <label>What colors combine to make green?</label>
    <choice>Red</choice>
    <choice>Purple</choice>
...
<choiceInput shuffleOrder>
  <label>Pick a letter (shuffled)</label>
  <choice>A</choice>
  <choice>B</choice>
  <choice>C</choice>
  <choice>D</choice>
...
<choiceInput preselectChoice="2">
  <label>Pick a letter (B is preselected)</label>
  <choice>A</choice>
  <choice>B</choice>
  <choice>C</choice>
</choiceInput>
...
<slider name="m" from="1" to="4">
  <label>Slider bound to choice</label>
</slider>
<choiceInput bindValueTo="$m">
  <label>Number bound to slider</label>
  <choice>1</choice>
  <choice>2</choice>
  <choice>3</choice>
  <choice>4</choice>
...
```doenet-editor-horiz
<choiceInput name="question1">
  <label>Pick an expression</label>
