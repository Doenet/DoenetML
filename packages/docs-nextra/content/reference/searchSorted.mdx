...
Write `allowUnsorted` on the operator and it reports where each target belongs among the values in
...
<p>The same position in one step: <searchSorted allowUnsorted target="80">$scores</searchSorted></p>
```
...
position again. The last line gets there directly: `allowUnsorted` hands the ordering to the
operator, which reports where 80 belongs among these values in ascending order without
rearranging them. `side` goes on meaning what it means.

Prefer `allowUnsorted` when the ordering is wanted only for the search. Keep a separate
