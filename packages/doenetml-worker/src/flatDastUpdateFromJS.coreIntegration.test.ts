 ...
 * `componentIdx -> name` map seeded from the initial render (for assertions that
 * mirror the bridge's action lookup), and a `drainAndConvertUpdates` helper for
 * the post-init pushes.
 */
async function createCapturingCore(doenetML: string) {
    ...
    core.setFlags({ ...defaultTestFlags });
    ...
    function drainAndConvertUpdates() {
...
describe.skipIf(!wasmAvailable)(
    ...
    () => {
        it("turns a textInput action into a FlatDast update map", async () => {
            const { core, resolvePathToNodeIdx, drainAndConvertUpdates } =
            ...
            const { batches, updates } = drainAndConvertUpdates();
...
<m name="mm">$ti</m>`,
            ...
            const { updates } = drainAndConvertUpdates();
