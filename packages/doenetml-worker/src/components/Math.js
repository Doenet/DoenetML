...
function calculateExpressionWithCodes({ dependencyValues, changes }) {
    ...
    for (let [
        childInd,
        child,
    ] of dependencyValues.stringMathChildren.entries()) {
        if (
            ...
            if (!dontGroup) {
                for (
                    let ind = childInd;
                    ind <= currentCompositeLastChildInd;
                    ind++
                ) {
                    if (
                        typeof dependencyValues.stringMathChildren[ind] ===
                        "string"
                    ) {
                        dontGroup = true;
                        break;
            ...
            if (dontGroup) {
                if (
                    compositeReplacementRange.length >
                    currentCompositeInd + 1
                ) {
        ...
        if (childInd === currentCompositeLastChildInd) {
            ...
            if (iString.length === 0) {
                ...
                if (!["{", "[", "(", "|", ","].includes(lastChar)) {
                    ...
                    let nextChild =
                        dependencyValues.stringMathChildren[childInd + 1];
