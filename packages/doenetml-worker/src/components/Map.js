...
export default class Map extends CompositeComponent {
    ...
    static returnStateVariableDefinitions() {
        ...
        stateVariableDefinitions.numIterates = {
            ...
            definition: function ({ dependencyValues }) {
                ...
                return {
                    setValue: { numIterates, minNIterates, sourcesChildNames },
                };
            },
        };
        ...
        stateVariableDefinitions.template = {
            ...
            definition: function ({ dependencyValues }) {
                ...
                let childrenOfTemplate =
                    templateChild.stateValues.serializedChildren;
                let template = {
    ...
    static async calculateReplacementChanges({
        ...
        if (currentMinNIterates < prevMinNIterates) {
            for (let ind = 0; ind < numSources; ind++) {
                for (
                    let ind2 = currentMinNIterates;
                    ind2 < prevMinNIterates;
                    ind2++
                ) {
                    if (
                        components[lrp.sourcesChildNames[ind][ind2]] ===
                        undefined
                    ) {
        ...
        if (currentMinNIterates < prevMinNIterates) {
            ...
            if (numReplacementsToAdd > 0) {
                ...
                for (
                    let iter = prevMinNIterates;
                    iter < currentMinNIterates;
                    iter++
                ) {
                    let res = await this.parallelReplacement({
                        component,
