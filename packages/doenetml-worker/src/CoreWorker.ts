...
export class CoreWorker {
     ...
     *
     * `initializeJavascriptCore` ends by releasing it — the JavaScript core
     * has consumed it by then, and the Rust core keeps its copy only for the
     * rust core-type's later `returnNormalizedDastRoot` calls — so a second
     * initialization with no `setSource` in between has nothing to initialize
     * from. `source_set` cannot say so: the source was set, and still is; what
