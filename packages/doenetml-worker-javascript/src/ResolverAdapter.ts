 ...
 * after each mutation.
 *
 * Stateless — the resolver lives outside this process. Holds a back-reference
 * to Core to read `_components` / `componentInfoObjects`, invoke the resolver
 * callbacks (`addNodesToResolver`, `deleteNodesFromResolver`,
 * `calculateRootNames`), append diagnostics, and notify
...
export class ResolverAdapter {
    ...
    async addReplacementsToResolver({
        ...
        if (
            ...
            this.core.addNodesToResolver
        ) {
            this.core.addNodesToResolver(flatFragment, indexResolution);
    ...
    addComponentsToResolver(components: any[], parentIdx: number): void {
        ...
        if (this.core.addNodesToResolver) {
            this.core.addNodesToResolver(flatFragment, "None");
    ...
    removeComponentsFromResolver(componentsToRemove: any[]): void {
        ...
        if (this.core.deleteNodesFromResolver) {
            this.core.deleteNodesFromResolver({
                nodes: flatElements,
            });
