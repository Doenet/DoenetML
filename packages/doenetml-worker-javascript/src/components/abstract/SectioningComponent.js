...
const sectionTitleStateKeys = ["completed", "inProgress", "notStarted"];

function titleStateKeyFromCredit(creditAchieved) {
    if (creditAchieved === 1) {
        return "completed";
    }
...
function resolveSectionTitleLightColorSpec({
    dependencyValues,
    usedDefault,
    ownColorName,
    parentColors,
...
function shouldEmitSectionTitleColorDiagnostic({ source }) {
    if (!source?.authored) {
        return false;
    }
    ...
    // Once an inherited authored color has already passed through a boxed or
    // collapsible ancestor, that earlier heading bar owns the diagnostic.
