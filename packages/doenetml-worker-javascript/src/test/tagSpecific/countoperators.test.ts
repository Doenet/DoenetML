...
describe("Counting operator tag tests @group4", async () => {
    ...
    describe("binEdges", async () => {
        it("reads back the cut points, so counts can be labeled", async () => {
            let { core, resolvePathToNodeIdx } = await createTestCore({
                doenetML: `
    <numberList name="v"><number>0.5</number><number>1.5</number><number>2.5</number></numberList>
    <binCounts name="b" bins="0 1 2 3">$v</binCounts>
    <p name="pCounts">$b</p>
    <p name="pEdges">$b.binEdges</p>
    <p name="pOne">$b.binEdge2</p>
    `,
            });
        ...
        it("is empty when no bins were given", async () => {
            let { core, resolvePathToNodeIdx } = await createTestCore({
                doenetML: `
    <binCounts name="b"><number>1</number></binCounts>
    <p name="p">$b.binEdges</p>
    `,
            });
    ...
    describe("repeated categories", async () => {
        it("each repeated category counts its own values, and it is a warning", async () => {
            const { warnings } = await messagesFor(`
    <numberList name="n">1 1 2</numberList>
    <p><tally categories="1 1 2">$n</tally></p>
    `);

            expect(
                warnings.some((m) =>
                    m.includes("names the same category more than once"),
                ),
            ).eq(true);
        });

        it("counts the repeated category the same in each slot", async () => {
            let { core, resolvePathToNodeIdx } = await createTestCore({
                doenetML: `
    <numberList name="n">1 1 2</numberList>
    <p name="p"><tally categories="1 1 2">$n</tally></p>
    `,
            });

            // Each slot reports the values equal to its own category, so the
            // two 1-slots both say 2 rather than the first taking them all.
            await expectText({
                core,
                resolvePathToNodeIdx,
                name: "p",
        ...
        it("distinct categories raise nothing", async () => {
            const { warnings } = await messagesFor(`
    <numberList name="n">1 1 2</numberList>
    <p><tally categories="1 2">$n</tally></p>
    `);
        ...
        it("inferred categories cannot repeat, so nothing is raised", async () => {
            const { warnings } = await messagesFor(`
    <numberList name="n">1 1 2</numberList>
    <p><tally>$n</tally></p>
    `);
