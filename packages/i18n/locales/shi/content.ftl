...
# in this catalog therefore writes three branches where `locales/zgh` and
# `locales/kab` write two, and the three Berber catalogs' plural rules are as
# different from each other as their scripts are. Plural categories are
# per-language CLDR data; a family and a script both say nothing about them.
#
