# Nauruan (dorerin Naoero) diagnostics. Translated from
...
# are two hundred and twenty whole sentences, and a sentence built entirely
# out of loans is an English sentence. So the messages below stand in English
# with their placeables, their variant keys and their argument names intact,
# waiting for a speaker: every `{ $attribute }`, every `[not-inline]`, every
# `[one]`/`*[other]` pair is already in the right place, and rewriting the
# prose around them changes nothing structural. That is a recorded debt of
# exactly the kind `locales/ewo` and `locales/vro` set the precedent for, and
# it is the largest one in the roster.
#
# Where English's `one`/`*[other]` pair differs only in the verb — "is
# ignored" against "are ignored" — both branches are kept, because a Nauruan
# rewrite may want the distinction even though the noun beside the count does
# not move. Nothing here writes a `[two]`, `[few]` or `[many]` branch.
#
# `math-embedded-input-shape-unsuitable` is the batch's new key and its four
# reason keys — `[not-inline]`, `[expanded]`, `[on-graph]` and the
# `*[relative-width]` default — are symbols the core passes in. They are
# copied byte for byte and must stay that way.
