# Meänkieli (Tornedalen Finnish) editor and language-server surfaces.
...
# national minority language of Sweden with a norm of its own rather than a
# dialect spelling of Finnish. In the words this file contains it parts from
# `locales/fi` by writing no d (eellinen, nähäksesi, maholliset), by iA for
# Finnish eA (oikia), by tt for Finnish ts (kattelin — the word for the viewer
# itself), by the illative h (kohtheesen), and by «oon» for Finnish «on».
#
...
editor-variant-previous = Valitse eellinen muunnelma
...
editor-accessibility-title =
    { $status ->
        ...
        [advisories] Napsauta { $action ->
            [close] sulkeaksesi
           *[open] avataksesi
        } saavutettavuusraportin. WCAG AA -rikkomuksia ei löytynyt, mutta saatavilla oon lisäsuosituksia saavutettavuudesta.
...
help-placeholder = Vie kohdistin tunnisteen nimen, attribuutin tai kohteen { $ref } päälle nähäksesi dokumentaation.
...
help-suggestions-footer = Paina { $shortcut } nähäksesi kaikki { $total } komponenttia.

help-name-summary = { $name } — { $summary }

help-ref-is-reference =
    { $line ->
        [none] { $ref } oon viittaus kohtheesen { $target }.
       *[other] { $ref } oon viittaus kohtheesen { $target } (rivi { $line }).
    }
...
help-property-is-reference =
    { $line ->
        [none] { $ref } oon viittaus kohteen { $element } ominaisuuteen { $property }.
       *[other] { $ref } oon viittaus kohteen { $element } ominaisuuteen { $property } (rivi { $line }).
    }
...
help-resolved-function-names = Ratkaistut funktioitten nimet:
