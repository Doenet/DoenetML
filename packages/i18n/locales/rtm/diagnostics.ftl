# Rotuman (Fäeag Rotuạm) diagnostics: the warnings and errors the viewer and
# the editor show. Selected by `uiLocale`, so these follow the reader rather
# than the document.
#
# DoenetML component, attribute and state-variable names — `through`,
# `endpoint`, `midpointOffset`, `numDimensions`, `selectFromSequence` and the
# rest — are part of the language, not prose, and stay in English exactly as
# written.
#
...
#
# **This is the file with the largest debt.** Every sentence here is English.
# Rotuman has no established vocabulary for the objects these messages talk
# about — a domain, an eigenvalue, a variant, a slope field — and a seed that
# coined two hundred and twenty of them would be publishing invention as
# record. What is Rotuman about this file is what is *not* wrong in it: the
# ids, the placeables, the variant keys and the diagnostic codes are intact, so
# a speaker rewriting a message has a frame that already works and only has to
# supply words.
#
# Every selector keeps English's branches, including the plural ones. Where the
# text is English, English's own plural is what a reader of it expects; a
# rewrite into Rotuman may collapse `one` and `*[other]` into one form, because
# a Rotuman noun does not change beside a numeral. The nested selects in
# `function-domain-insufficient-dimensions`,
# `function-iterates-input-output-mismatch` and
# `field-function-wrong-num-outputs` are kept in the shape English gives them
# so that no branch goes missing on the way.
