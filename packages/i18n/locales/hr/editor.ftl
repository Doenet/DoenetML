# Croatian editor and language-server surfaces. Translated from
...
#
# Croatian counts in three plural categories, but only a message that prints
# the number beside a noun needs all three. `help-coordinates` never shows its
# count — it decides a heading's singular against its plural, and Croatian has
...
editor-update-viewer-title =
    { $shortcut ->
        [none] { $word } preglednik
       *[other] { $word } preglednik { $shortcut }
    }
...
editor-variant-filter = Filtar…
editor-variant-next = Odaberi sljedeću varijantu
editor-variant-previous = Odaberi prethodnu varijantu
...
editor-diagnostic-line = Redak br. { $line }

editor-no-errors = Nema pogrešaka
...
help-placeholder = Postavite pokazivač na ime oznake, atribut ili { $ref } za dokumentaciju.

help-unsupported-ref-chain = Pomoć za višedijelne upute poput { $example } još nije podržana.

help-unresolved-ref =
    { $reason ->
        [notFound] Nije pronađen objekt za uputu: { $ref }.
        [multiple] Pronađeno je više objekata za uputu: { $ref }.
       *[indeterminate] Objekt za { $ref } nije se mogao odrediti.
    }
...
help-ref-is-reference =
    { $line ->
       ...
       *[other] { $ref } je uputa na { $target } (redak { $line }).
    }

help-ref-derived-from =
    { $line ->
        [none] Uveo ju je { $owner } kao { $role }.
       *[other] Uveo ju je { $owner } u retku { $line } kao { $role }.
    }

help-property-is-reference =
    { $line ->
        [none] { $ref } je uputa na svojstvo { $property } elementa { $element }.
       *[other] { $ref } je uputa na svojstvo { $property } elementa { $element } (redak { $line }).
    }
...
help-reset-list = Popis za poništavanje na ovom polju:
