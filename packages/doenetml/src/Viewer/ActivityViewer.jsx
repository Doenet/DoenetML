...
export function ActivityViewer({
    ...
    updateCreditAchievedCallback = null,
    updateActivityStatusCallback = null,
    updateAttemptNumber = null,
    pageChangedCallback = null,
    paginate,
    showFinishButton = false,
    cidChangedCallback = null,
    checkIfCidChanged = null,
    setActivityAsCompleted = null,
    setIsInErrorState = null,
    apiURLs = {},
    ...
    useEffect(() => {
        if (currentPage > 0 && nPages > 1) {
            ...
            if (stillNeedToScrollTo.current) {
                document
                    .getElementById(stillNeedToScrollTo.current)
                    ?.scrollIntoView();
    ...
    useEffect(() => {
        ...
        if (currentLocationKey.current !== location.key) {
            ...
            if (previousLocations.current[location.key]) {
                ...
                if (
                    previousLocations.current[location.key]
                        ?.lastScrollPosition !== undefined
                ) {
                    scrollableContainer.scroll({
                        top: previousLocations.current[location.key]
                            .lastScrollPosition,
                    });
                }
            }
    ...
    function clickPrevious() {
        ...
        let event = {
            ...
            object: {
                objectType: "button",
                objectname: "previous page button",
            },
                ...
                >
                    ...
                    >
                        <div style={{ width: "240px" }}>
                            <ActionButton
                                onClick={() =>
                                    setFinishAssessmentMessageOpen(true)
                                }
            ...
            if (err.doenetMLrange?.lineBegin !== undefined) {
                rangeMessage = (
                    <>
                        ...
                        <em>
                            {"Found on " +
                                printDoenetMLrange(err.doenetMLrange) +
                                "."}
                        </em>
                    </>
                );
