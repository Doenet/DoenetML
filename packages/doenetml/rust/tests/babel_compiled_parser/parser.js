...
function parse(inText) {
    return _doenet.parser.parse(inText).cursor();
}
/**
...
function parseAndCompile(inText) {
        ...
        if (cursor.name === "OpenTag") {
            ...
            while (cursor.nextSibling()) {
                ...
                if (cursor.name !== "Attribute") {
                    ...
                    let errorEnd = cursor.to; // console.error(cursor);
                ...
                cursor.firstChild();
                let attrName = inText.substring(cursor.from, cursor.to); //skip the name and equals sign
                ...
                if (cursor.nextSibling() === false) {
                    ...
                    cursor.nextSibling(); //boundry fuddling to ignore the quotes
            ...
            let range = {
                selfCloseBegin: cursor.from,
                selfCloseEnd: cursor.to,
