...
export default function PageViewer(props) {
    ...
    async function saveInitialRendererStates() {
        ...
        try {
            let result = await returnAllPossibleVariants({
                doenetId: props.doenetId,
                cid,
                flags: props.flags,
            });
    ...
    if (saveStatesWorker) {
        saveStatesButton = (
            <button onClick={() => cancelSaveInitialRendererStates()}>
                Cancel saving initial renderer states
            </button>
        );
    } else {
        saveStatesButton = (
            <button onClick={() => saveInitialRendererStates()}>
    ...
    return (
        <ErrorBoundary setIsInErrorState={props.setIsInErrorState}>
            {noCoreWarning}
            {/* <p>{saveStatesButton}</p> */}
            <div style={pageStyle}>{documentRenderer}</div>
        </ErrorBoundary>
    );
}
