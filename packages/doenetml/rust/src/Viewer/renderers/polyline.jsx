...
export default React.memo(function Polyline(props) {
    let { name, SVs, actions, sourceOfUpdate, callAction } =
    ...
    function createPolylineJXG() {
        ...
        if (SVs.applyStyleToLabel) {
            jsxPolylineAttributes.label.strokeColor =
        ...
        newPolylineJXG.on("down", function (e) {
            ...
            pointsAtDown.current = newPolylineJXG.points.map((x) => [
                ...x.scrCoords,
            ]);
        });
    ...
    if (board) {
        if (!polylineJXG.current) {
            ...
            if (SVs.nVertices > previousNVertices.current) {
                for (
                    let i = previousNVertices.current;
                    i < SVs.nVertices;
                    i++
                ) {
                    pointsJXG.current.push(
            ...
            if (
                polylineJXG.current.visProp.strokecolor !==
                SVs.selectedStyle.lineColor
            ) {
                polylineJXG.current.visProp.strokecolor =
                    SVs.selectedStyle.lineColor;
                polylineJXG.current.visProp.highlightstrokecolor =
                    SVs.selectedStyle.lineColor;
            }
            if (
            ...
            for (let i = 0; i < SVs.nVertices; i++) {
                if (layerChanged) {
                    pointsJXG.current[i].setAttribute({
                        layer: polylineLayer + 2,
                    });
                }
