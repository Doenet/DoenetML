...
describe("Graph Reference Test 2", function () {
    ...
    `,
        ...
        cy.window().then(async (win) => {
            ...
            let graph5 =
                stateVariables[
                    stateVariables["/graph5"].replacements[0].componentName
                ];
            let graph6 =
                stateVariables[
                    stateVariables["/graph6"].replacements[0].componentName
                ];
            let graph7 =
                stateVariables[
                    stateVariables["/graph7"].replacements[0].componentName
                ];
            let graph8 =
                stateVariables[
                    stateVariables["/graph8"].replacements[0].componentName
                ];
            let graph9 =
                stateVariables[
                    stateVariables["/graph9"].replacements[0].componentName
                ];
            let graph10 =
                stateVariables[
            ...
            let graph1A =
                stateVariables[
                    stateVariables[
                        stateVariables[
                            stateVariables["/sbsgroup2"].replacements[0]
                                .componentName
            ...
            cy.window().then(async (win) => {
                ...
                for (let vector of vectors) {
                    expect(stateVariables[vector].stateValues.tail).eqls([
                        ...ov_t,
                    ]);
                    expect(stateVariables[vector].stateValues.head).eqls([
                        ...ov_h,
                    ]);
                    expect(
                ...
                for (let displacement of displacementsA) {
                    expect(stateVariables[displacement].stateValues.tail).eqls([
                        ...d1_t,
                    ]);
                    expect(stateVariables[displacement].stateValues.head).eqls([
                        ...d1_h,
                    ]);
                    expect(
                ...
                for (let dTail of displacementTails) {
                    expect(stateVariables[dTail].stateValues.xs).eqls([
                        ...d1_t,
                    ]);
                }
                for (let dHead of displacementHeads) {
                    expect(stateVariables[dHead].stateValues.xs).eqls([
                        ...d1_h,
                    ]);
                }
            });
