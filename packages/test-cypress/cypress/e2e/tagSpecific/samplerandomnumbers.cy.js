...
describe("SampleRandomNumbers Tag Tests", function () {
        ...
        cy.window().then(async (win) => {
            ...
            let samples = stateVariables["/_map1"].replacements.reduce(
                (a, c) => [
                    ...a,
                    ...stateVariables[
                        stateVariables[c.componentName].replacements[0]
                            .componentName
                    ].replacements.map(
                        (y) =>
                            stateVariables[y.componentName].stateValues.value,
                    ),
                ],
                [],
            );
    ...
    `,
        ...
        cy.window().then(async (win) => {
            ...
            for (let sample of samples) {
                expect([0.5, 1.5, 2.5, 3.5, 4.5, 5.5].includes(sample)).eq(
                    true,
                );
            }
    ...
    `,
        ...
        cy.window().then(async (win) => {
            ...
            for (let sample of samples) {
                expect([-3, -2, -1, 0, 1, 2, 3, 4, 5].includes(sample)).eq(
                    true,
                );
            }
    ...
    `,
        ...
        cy.window().then(async (win) => {
            ...
            expect(me.math.variance(sample2numbers, "uncorrected")).closeTo(
                16,
                8,
            );
        });
        ...
        cy.window().then(async (win) => {
            ...
            expect(me.math.variance(sample2numbersb, "uncorrected")).closeTo(
                16,
                6,
            );
