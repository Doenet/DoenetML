...
describe("Numberlist Tag Tests", function () {
    ...
    it("dynamic maximum number", () => {
      ...
      `,
        ...
        cy.window().then(async (win) => {
            let stateVariables = await win.returnAllStateVariables1();
            expect(stateVariables["/nl1"].stateValues.numbers).eqls([
                1, 2, 3, 4, 5,
            ]);
            expect(stateVariables["/nl2"].stateValues.numbers).eqls([
                1, 2, 3, 4, 5,
  ...
  <p name="pcopynlml">$nlml</p>
    ...
    `,
        ...
        cy.window().then(async (win) => {
            ...
            let numberlist1a =
                stateVariables[
                    stateVariables["/numberlist1a"].replacements[0]
                        .componentName
                ];
            let numberlist3 =
                stateVariables[
            ...
            expect(stateVariables["/_numberlist2"].stateValues.numbers[2]).eq(
                2,
            );
            expect(stateVariables["/_numberlist2"].stateValues.numbers[3]).eq(
                3,
            );
            expect(stateVariables["/_numberlist2"].stateValues.numbers[4]).eq(
                5,
            );
            expect(stateVariables["/_numberlist2"].stateValues.numbers[5]).eq(
                1,
            );
            expect(stateVariables["/_numberlist2"].stateValues.numbers[6]).eq(
                2,
            );
            expect(stateVariables["/_numberlist2"].stateValues.numbers[7]).eq(
                3,
            );
    ...
    `,
        ...
        cy.get(cesc2("#/mi1") + " textarea").type("{end}3{enter}", {
            force: true,
        });
