...
#[darling(attributes(component_attribute))]
...
#[darling(attributes(component_attribute), supports(enum_any))]
...
pub fn attribute_state_var_derive(input: TokenStream) -> TokenStream {
    ...
    for (explicit_type, impls) in &variant_impls {
        ...
        for impl_ in impls {
            accessor_impls.push(impl_.to_match_case(&enum_name));
        }

        let accessor_impls = accessor_impls.into_iter().collect::<TokenStream2>();
