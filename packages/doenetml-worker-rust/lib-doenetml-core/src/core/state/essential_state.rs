...
pub type EssentialProp = PropViewMutEnum;
...
pub enum EssentialDataOrigin {
    /// Essential data was generated because a state variable make a preliminary value data query
    ...
    /// Essential data was generated for a state variable because a component did not have children.
    ///
    /// Unnamed `usize` field is the index of the state variable requesting the children.
    ...
    /// Essential data was generated for a state variable because an attribute did not have children.
    ///
    /// Unnamed `usize` field is the index of the state variable requesting the children.
...
//     essential_data: &Vec<HashMap<EssentialDataOrigin, EssentialProp>>,
// ) -> bool {
...
pub fn create_essential_data_for(
    ...
    let essential_state = match initial_values {
        InitialEssentialData::Single {
            value,
            came_from_default,
        } => EssentialProp::new_with_value(value, came_from_default),
    };
