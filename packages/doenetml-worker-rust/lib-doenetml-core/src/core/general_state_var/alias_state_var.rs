...
pub struct AliasRequiredData<T>
where
    T: Default + Clone,
{
...
impl<T> StateVarUpdater<T, AliasRequiredData<T>> for StateVarAlias
where
    ...
    <StateVarView<T> as TryFromState<StateVarViewEnum>>::Error: std::fmt::Debug,
{
    ...
    fn calculate<'a>(&self, data: &'a AliasRequiredData<T>) -> StateVarCalcResult<'a, T> {
