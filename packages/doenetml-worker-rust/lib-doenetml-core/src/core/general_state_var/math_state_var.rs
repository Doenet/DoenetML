...
impl StateVarUpdater<MathExpr, RequiredData> for MathStateVar {
    ...
    fn calculate<'a>(&self, data: &'a RequiredData) -> StateVarCalcResult<'a, MathExpr> {
        match data.math_or_string.len() {
            0 => StateVarCalcResult::Calculated(MathExpr::default()),
            1 => {
                match &data.math_or_string[0] {
                    MathOrString::Math(math_value) => {
                        ...
                        StateVarCalcResult::From(math_value)
                    }
                    MathOrString::String(string_value) => {
                        ...
                        StateVarCalcResult::Calculated(math_expr)
                    }
                }
            }
            _ => {
                // TODO: implement
                StateVarCalcResult::Calculated(MathExpr::default())
            }
        }
    }
    ...
    fn invert(
        ...
        state_var: &StateVarView<MathExpr>,
