...
impl FlatRoot {
    ...
    ///
    /// Internal invariant, not an authoring error: `idx` comes from a node this
    /// merge just created as an element, so a non-element here means a caller
    /// passed the wrong index -- a bug in the core rather than in the document.
    /// Audited for #1921 against a corpus of documents aimed at exactly this
    /// (see `dast/panic_reachability.test.rs`); none reached it.
    ...
    ///
    /// Internal invariant: only the `DastFunctionMacro` arm reaches this, and
    /// it has just written a `FlatNode::FunctionRef` at `idx`. A document
