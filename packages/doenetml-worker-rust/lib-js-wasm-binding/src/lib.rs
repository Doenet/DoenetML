...
/// The root names that changed since the previous call to `update_root_names`
/// (or all of them, if `report_all` was set),
/// as `[node index, new root name]` pairs, where a `null` name means the node
/// no longer has a root name.
#[derive(Debug, Serialize, Deserialize, Tsify)]
#[tsify(into_wasm_abi, from_wasm_abi)]
pub struct RootNameChanges {
...
impl PublicDoenetMLCore {
    ...
    pub fn update_root_names(&mut self, report_all: bool) -> RootNameChanges {
        RootNameChanges {
            changes: self.core.update_root_names(report_all),
        }
    }
