...
export default React.memo(function Vector(props) {
    ...
    function createVectorJXG() {
        ...
        function onDragHandler(e, i) {
            ...
            if (performMove) {
                ...
                newVectorJXG.point1.coords.setCoordinates(
                    JXG.COORDS_BY_USER,
                    lastPositionsFromCore.current[0]
                );
                newVectorJXG.point2.coords.setCoordinates(
                    JXG.COORDS_BY_USER,
    ...
    if (board) {
        if (Object.keys(vectorJXG.current).length === 0) {
            ...
            if (
                vectorJXG.current.visProp.strokecolor !==
                SVs.selectedStyle.lineColor
            ) {
                vectorJXG.current.visProp.strokecolor =
                    SVs.selectedStyle.lineColor;
                vectorJXG.current.visProp.highlightstrokecolor =
                    SVs.selectedStyle.lineColor;
            }
            if (
