...
A `gaussian` whose spread or centre describes no distribution now reports `NaN` for `mean`, `variance` and `standardDeviation`, as the other distributions already did, instead of reporting a plausible-looking spread beside `NaN` samples; an infinite spread is recognised as unusable rather than sampled. The explanation also survives a reload, and `<selectRandomNumbers>` holds its distribution parameters fixed alongside the selection they produced.
