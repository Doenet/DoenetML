...
The legend is drawn as soon as a series has a label, keyed off the bars themselves so its swatches cannot disagree with the colors they name. `legend="false"` suppresses it, and `legendPosition` picks the corner — the same four corner names `<legend>` already uses inside a `<graph>`. `$chart.showLegend` reports whether a legend is drawn rather than whether one was asked for, so it is false for a chart whose series carry no labels.
