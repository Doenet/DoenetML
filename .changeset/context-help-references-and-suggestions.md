...
- When the cursor is in an element's body or in empty top-level space, the panel now suggests components and snippet templates to try instead of going blank, and points to Ctrl+Space for the full list. The same ranking — per-container hand-picked entries first, then by how directly a child is allowed (dropping adapter-only children unless picked), then alphabetical, with snippets clustered next to the element they're tied to — also drives the autocomplete dropdown's order, so the two surfaces stay in lockstep.
