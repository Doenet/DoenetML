...
Filipino is where a plural category is least like a count. CLDR splits `fil` by the linker a numeral takes — `one` is every number whose Tagalog word ends in a vowel and takes `-ng`, and `other` is 4, 6, 9 and anything ending in them, which take the separate `na` — so `one` catches five and `other` catches four. Filipino marks number with the free word `mga` instead, and a message that wants a singular selects `[1]` by number.
