...
`<slopeField>y - x</slopeField>` draws the slope field of a differential equation as tick marks on a lattice; `<vectorField>(y, -x)</vectorField>` draws a two-output function as arrows, either scaled by magnitude or normalized to show direction alone. Both take their function as a bare expression child, as an expression written into the `function` attribute — `<vectorField function="(y, -x)" />` — or from a `<function>` referenced with that attribute, which may take one input or two. An expression written either of the first two ways is read as a function of both `x` and `y`; a referenced function keeps whatever variables its own author named. Given a function with the wrong number of outputs for the component, a field draws nothing and warns, naming the sibling component that does want it. Given the function both ways at once, it uses the one written inside the component and warns that the attribute was ignored. The lattice they are sampled on is set by `dx`, `dy`, `xoffset` and `yoffset`, which mirror `<pegboard>`'s attributes of the same names, along with `markLength` (measured in pixels) and `maxMarks`. A slope mark is centered on its lattice point, sampling the tangent line through it, while a vector field's arrow has its tail on the lattice point so it shows the vector at that point.
