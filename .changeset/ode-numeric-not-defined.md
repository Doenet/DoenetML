...
Nothing under `packages/` needed to change, which is why the regression test is a Cypress spec rather than a Vitest one: Vitest runs under Node, where `global` exists and numeric registers itself, so the suite stayed green throughout. Only a real browser reproduces it.
