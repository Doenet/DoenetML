...
- **A value that is not a number now reads as `NaN` rather than as zero.** The engine reports "I
  ...
  a `<line>` through an undefined point — the result is `NaN`, so an undefined point is not drawn
  at the origin and an unclampable value does not report as the lower bound. `±Infinity` is still a
  value and still clamps.
...
made, but the bundle carries it: the engine is 2.40 MiB uncompressed and 788 kB gzipped, against
