...
A pegboard listens for the graph's bounding box changing so that it can re-tile as the graph is panned or zoomed. That listener was never removed. The board outlives a pegboard taken out of the document — one inside a `<conditionalContent>` that switches off, say — so the next pan or zoom found no pegs and built a fresh set. Those pegs belonged to no component, so nothing could ever remove them; only reloading cleared them. The listener now goes when the component does, and is registered once rather than on each rebuild.
