...
**Corsican is the roster's first locale whose endonym is its English name.** `supportedLocales.ts` asks `Intl.DisplayNames` for a language's name in itself and CLDR has no Corsican-language data to answer with, so the roster offers "Corsican" twice. That is a gap in CLDR, and nothing here hand-writes around it — the same rule that makes `ny` read "Nyanja".
