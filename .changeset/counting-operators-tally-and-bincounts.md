...
Name the categories with `categories` to fix which are counted and in what order; omit it and the categories are the distinct values present, in sorted order — sorted rather than first-seen, so the same data reads the same way however it arrived. Either way they are readable back as `.categories`, so a table or a chart is driven off the same component that did the counting.

A category is read to match the values, so there is nothing to declare for it: `categories="apple fig"` counts words, `categories="true false"` counts booleans, and `categories="1/2 1"` counts halves — an expression written as a category names the number it evaluates to when the values are numbers, while still labeling its count as it was written. Categories given as components rather than as text keep every digit they have, so a third counts thirds rather than the `0.333` it displays as. `type` is what bare string children are read as — `<tally type="text">apple fig apple</tally>` — and a tally whose values arrive as components needs none. Written out, it also decides how `categories` is read, which is otherwise text: `<tally type="math" categories="2x">` matches a value written `x*2`, and `<tally type="boolean" categories="true false">` labels its counts `true, false`.
...
**`<binCounts>`** counts how many values fall into each interval between the cut points given by `bins`, which is what a histogram of a continuous quantity needs. `n + 1` cut points define `n` intervals. Bare numbers are read as values, the way `<sum>` reads them, so `<binCounts bins="0 1 2">0 1/2 1 3/2 2</binCounts>` counts five of them.
...
Six situations are reported rather than left silent. `<tally>` raises info when values matched none of the *declared* categories and so were counted nowhere — info rather than a warning, since a list fed by an input legitimately holds non-categories while a student is typing, and the diagnostics queue is append-only. `<tally>` also warns when `categories` names the same category more than once: each count then reports the values equal to its own category, so the repeated ones all report the same number and the counts can exceed the number of values. `<binCounts>` warns when `bins` is missing, when it holds fewer than the two cut points an interval needs, when a cut point is not a number or is smaller than the one before it (a bin running backwards has no count; two equal cut points are accepted, and name an empty bin between other bins — at either end the always-included outermost cut point wins, so `bins="0 1 1"` counts a value of `1` in the last bin rather than nowhere), and when its values are not all numeric.
...
The cut points read back as `.binEdges`, so whatever displays the counts can say what interval each covers.
