...
And fixes a pre-existing duplication in the editor hover: parser-emitted DAST errors were surfaced once by the LSP and again by the worker's `_error`-component pipeline, so the hover tooltip showed the same message twice even though only one squiggle was drawn. The LSP now dedupes diagnostics by severity + range + message before sending them to the editor.
