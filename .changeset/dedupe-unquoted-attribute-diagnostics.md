...
And fixes a pre-existing duplication in the editor hover: parser-emitted DAST errors were surfaced once by the LSP and again by the worker's `_error`-component pipeline once the viewer ran, so the hover tooltip showed the same message twice even though only one squiggle was drawn. The fix is in two layers — the worker-side bridge now converts DAST positions (`{line, column}`, 1-indexed) to proper LSP `Range`s (`{line, character}`, 0-indexed) when it pushes diagnostics back to the LSP, and the LSP then dedupes by severity + range + message before sending to the editor. Before the conversion the two copies had different range shapes and slipped past the dedupe.
