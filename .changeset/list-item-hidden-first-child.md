...
Known limitation, unchanged by this and tracked separately as [#1673](https://github.com/Doenet/DoenetML/issues/1673): an `<li>` that leads with a box whose baseline is its bottom edge — a `<graph>`, `<image>`, `<video>` or `<figure>` — still has its number drawn at the bottom of that box rather than beside its top, and one leading with a `<matrixInput>`, or with a `<tabular>` whose first cell holds a `<p>`, still has it drawn a row low. Only the number's vertical position is affected; the top-margin suppression described in the accompanying entry, which is what closes the gap between items of different shapes, applies to all of them.
