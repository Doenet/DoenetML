...
An `<li>` leading with a box that offers no first line of text — a `<graph>`, `<image>`, `<video>`, `<figure>` or `<tabular>` — had its number drawn at the bottom of that box rather than beside its top. That was tracked separately as [#1673](https://github.com/Doenet/DoenetML/issues/1673), and the entry for it in this same release draws the number beside the top of such a lead instead. Two of the shapes named there are unchanged on purpose: an item leading with a `<matrixInput>` keeps its number on the row its label sits on, which is where it belongs, and a `<p>` alone in a table cell still keeps its own top margin, so a leading `<tabular>` built that way starts its cell text a row below the number.
