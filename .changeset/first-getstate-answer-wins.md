...
Rebuilding the document twice also cost a full second core start on every restore, which is now spent only when a state actually arrives to rebuild from.
