...
An index inside a reference in content already followed its copy; one inside a reference in an attribute now does too, so each copied button changes the entry of `m` the button it was copied from changes.
