...
What the host says is now a notice beside the document, in the reader's language and carrying the host's own words. The document, and the work in it, stay where they are, and the host is not told the document failed.

The failure pane also has a precedence rule instead of an arrival order. It is reserved for failures that leave no document at all — a core that never started, saved state that could not be read — and when there is more than one thing to say, an attributable cause outranks the generic "This document could not be started", which keeps the host's account beneath it rather than in place of it. The **Try again** button stays with the failure it addresses, rather than following whichever message settled last.
