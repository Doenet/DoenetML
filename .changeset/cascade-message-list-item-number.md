...
The message is now the child such a step lines its number up with, which is what it always was on the screen. Every step a cascade is holding back gets this, not just the next one in line. Nothing changes once the step is revealed: the message is hidden then, and the content behind it leads as before. Nothing changes for a step with a title or a box of its own either — those draw their number in a heading, with the message below it, exactly as they did.
