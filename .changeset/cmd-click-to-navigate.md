...
- VS Code editor → preview: the preview no longer follows the cursor by default. The new `Scroll Doenet Preview to Cursor` command scrolls it on request, bound to `Ctrl+Alt+P` (`Cmd+Alt+P` on macOS) and rebindable from the Keyboard Shortcuts UI — the same chord as the web editor. The VS Code API exposes no mouse modifiers for editor clicks, so a keybinding is the closest equivalent of the web editor's Cmd/Ctrl+click. Set `doenet.preview.scrollPreviewWithEditor` to restore the old follow-the-cursor behavior.
