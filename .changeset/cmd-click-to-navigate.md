...
- `DoenetEditor` editor → preview: the debounced follow-the-cursor scroll is replaced by Cmd/Ctrl+click on a spot in the source, which scrolls the preview to the element rendered from that offset. Typing and plain cursor moves never scroll the preview. The code editor reads that same modifier as "add another cursor", so the gesture now leaves a single cursor at the clicked spot instead: multiple cursors can no longer be placed with the mouse, only with `Cmd/Ctrl+D`.
