...
The id now decides only whether a reply belongs to this viewer, and the payload decides what it is. An error either quoting the open request's id or carrying none is surfaced; one quoting a different id is ignored, since it answers a request some rebuild has already replaced. An error still does not close the request, so a page holding a second listener — the `@doenet/standalone` coordinator beside a book's own persistence layer — can still restore the document after the first answerer has failed.
