...
column before the reader jumped to the right one, so cosine and tangent were
...
expression explorer. Walking into a subexpression rewrites the text along with
...
same treatment, since it has the same problem; a host that would rather keep
MathJax's output untouched can pass `exposeSpeechAsText: false` to
`loadMathJax`.
