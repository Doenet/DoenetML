import { u as q, C as U, g as Y, j as _, B as I } from "./index-8862516e.js";
import K, { useRef as c, useContext as Q, useEffect as Z } from "react";
import { BoardContext as $ } from "./graph-de465854.js";
import "react-dom";
...
const ce = K.memo(function E(M) {
  ...
  const i = Q($);
  let x = c(!1), B = c(!1), a = c(!1), s = c(null), l = c(null), D = c(null), P = c(null), R = c(!1), X = c(!1);
  ...
  function L() {
    ...
    let u = i.create(
      "button",
      [0, 0, C, () => p({ action: m[t.clickAction] })],
      o
    );
    u.isDraggable = !X.current, u.on("down", function(n) {
      x.current = [n.x, n.y], B.current = [...r.coords.scrCoords], a.current = !1;
    }), u.on("hit", function(n) {
      a.current = !1;
    }), u.on("up", function(n) {
      a.current && (p({
      ...
      (!d || Math.abs(n.x - x.current[0]) > 0.1 || Math.abs(n.y - x.current[1]) > 0.1) && (a.current = !0);
      ...
      if (d) {
        ...
        s.current = (B.current[1] + n.x - x.current[0] - N[1]) / i.unitX, l.current = (N[2] - (B.current[2] + n.y - x.current[1])) / i.unitY;
      } else
  ...
  return t.hidden ? null : /* @__PURE__ */ _.jsxs("div", { id: b, margin: "12px 0", style: { display: "inline-block" }, children: [
    ...
    /* @__PURE__ */ _.jsx(
      ...
      {
        id: b + "_button",
        onClick: () => p({ action: m[t.clickAction] }),
        disabled: t.disabled,
        value: C,
