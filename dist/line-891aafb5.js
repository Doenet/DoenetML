import { u as N, d as I, e as V, j as g, t as q, M as z } from "./index-dce15b2c.js";
import K, { useContext as Q, useRef as l, useEffect as A } from "react";
import { BoardContext as Z, LINE_LAYER_OFFSET as E } from "./graph-b13204d4.js";
const re = K.memo(function m(S) {
  var J;
  let { name: v, id: L, SVs: e, actions: s, callAction: c } = N(S);
  m.ignoreActionsWithoutCore = () => !0;
  const a = Q(Z);
  let t = l({}), y = l(null), k = l(null), w = l(!1), O = l(!1), p = l(!1), R = l(null), _ = l(null), u = l(null), D = l(null), P = l(!1), x = l(!1), F = l(!1);
  ...
  function Y() {
    ...
    i.isDraggable = !x.current, i.on("drag", function(r) {
      ...
      if ((!n || Math.abs(r.x - y.current[0]) > 0.1 || Math.abs(r.y - y.current[1]) > 0.1) && (p.current = !0), u.current = [], n) {
        var o = a.origin.scrCoords;
        for (let h = 0; h < 2; h++) {
          let T = (k.current[h][1] + r.x - y.current[0] - o[1]) / a.unitX, H = (o[2] - (k.current[h][2] + r.y - y.current[1])) / a.unitY;
          u.current.push([T, H]);
        }
      } else
        u.current.push([
      ...
      p.current = !1, y.current = [r.x, r.y], k.current = [
    ...
    if (Object.keys(t.current).length === 0)
      ...
      if (r != R.current && (t.current.setAttribute({ withlabel: r }), R.current = r), t.current.needsUpdate = !0, t.current.update(), t.current.hasLabel)
        if (t.current.label.needsUpdate = !0, e.applyStyleToLabel ? t.current.label.visProp.strokecolor = i : t.current.label.visProp.strokecolor = "var(--canvastext)", e.labelPosition !== _.current) {
          let n, o, h;
          e.labelPosition === "upperright" ? (h = [5, 5], n = "left", o = "bottom") : e.labelPosition === "upperleft" ? (h = [-5, 5], n = "right", o = "bottom") : e.labelPosition === "lowerright" ? (h = [5, -5], n = "left", o = "top") : (h = [-5, -5], n = "right", o = "top"), t.current.label.visProp.anchorx = n, t.current.label.visProp.anchory = o, t.current.label.visProp.offset = h, _.current = e.labelPosition, t.current.label.fullUpdate();
        } else
