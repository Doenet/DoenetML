import { u as ne, R as re, r as se, j as l, E, M as H, C as ie } from "./index-dce15b2c.js";
import ue, { useRef as K, useState as B, useEffect as J } from "react";
import D from "styled-components";
import { A as q } from "./ActionButtonGroup-fdd0712e.js";
...
`;
function S(a, u, b, t) {
  let e, s = Math.max(Math.abs(t.firstItem), Math.abs(t.lastItem)), m = Math.round(Math.log(s) / Math.log(10));
  s === 0 && (m = 1);
  let O = 5 - m;
  if (a.length === 0) {
    let r = [
  ...
  if (t.width.size > e * M)
    if (a.length === 0) {
      let r = [], n = [], c = Math.max(Math.abs(t.firstItem), Math.abs(t.lastItem)), h = Math.round(Math.log(c) / Math.log(10));
      c === 0 && (h = 1);
      let v = 5 - h;
    ...
    let r, n;
    if (a.length === 0) {
      let c = Math.floor(t.width.size / e), h = t.lastItem - t.firstItem, v = h / (c + 1), p = Math.max(Math.abs(t.firstItem), Math.abs(t.lastItem)), w = Math.round(Math.log(p) / Math.log(10)), $ = 1 - w, f = Math.max(
        I(v, $),
        10 ** -$
      ), j = Math.floor(h / f) + 1, y = 5 - w;
      n = [...Array(j).keys()].map(
        (A) => t.from + f * A
      ), r = n.map(
      ...
      ), v = Math.floor((t.numItems - 1) / h + 1e-10) + 1;
      r = [...Array(v).keys()].map(
        (f) => Math.round(h * f)
      );
    ...
    return [
      r.map((c, h) => /* @__PURE__ */ l.jsx(T, { x: `${c * u}px` }, n[h])),
      r.map((c, h) => /* @__PURE__ */ l.jsx(k, { x: `${c * u}px`, children: n[h] }, n[h]))
    ];
  } else
    return [
...
function ee(a, u, b) {
  ...
  if (b.width.size > t * e)
    return [
      a.map((s, m) => /* @__PURE__ */ l.jsx(T, { x: `${m * u}px` }, s)),
      a.map((s, m) => /* @__PURE__ */ l.jsx(k, { x: `${m * u - 3}px`, children: s }, s))
    ];
  if (b.width.size < t)
    return [
      a.map((s, m) => m == 0 ? /* @__PURE__ */ l.jsx(T, { x: `${m * u}px` }, s) : ""),
      a.map((s, m) => {
        if (m == 0)
          return /* @__PURE__ */ l.jsx(k, { x: `${m * u - 3}px`, children: s }, s);
        if (m == 2)
          return /* @__PURE__ */ l.jsx(k, { x: `${m * u - 3}px`, children: "..." }, s);
      })
    ];
  if (b.width.size < t * e)
    return [
      a.map((s, m) => /* @__PURE__ */ l.jsx(T, { x: `${m * u}px` }, s)),
      a.map((s, m) => m == 0 || e === m + 1 ? /* @__PURE__ */ l.jsx(k, { x: `${m * u - 3}px`, children: s }, s) : /* @__PURE__ */ l.jsx(k, { x: `${m * u - 3}px`, children: s.length < 3 ? s : s.substr(0, 3) + "..." }, s))
    ];
}
function te(a, u, b) {
  return b + a / u;
...
const me = ue.memo(function a(u) {
  let { name: b, id: t, SVs: e, actions: s, ignoreUpdate: m, rendererName: O, callAction: M } = ne(u);
  a.baseStateVariable = "index";
  const r = K(null), n = re(se(O)), [c, h] = B(0), v = K(!1), [p, w] = B(0), $ = e.type === "text" ? 0 : e.firstItem;
  ...
  if (J(() => {
    ...
    !v.current && !m && (y(e.index), e.type !== "text" ? h(e.index / (e.numItems - 1) * e.width.size) : h(e.index * f));
  }, [e.index]), e.hidden)
    return null;
  if (e.disabled) {
    let x = "";
  ...
  function A(x) {
    if (v.current = !0, document.addEventListener("mousemove", W), document.addEventListener("mouseup", V), h(x.nativeEvent.clientX - p), e.type !== "text") {
      let i = te(
  ...
  function V(x) {
    if (document.removeEventListener("mousemove", W), document.removeEventListener("mouseup", V), !!v.current)
      if (v.current = !1, e.type !== "text") {
        ...
        i = Math.max(0, Math.min(e.numItems - 1, i)), y(i), h(i * f), n((d) => {
          let o = { ...d };
          return o.ignoreUpdate = !0, o;
        }), M({
          action: s.changeValue,
