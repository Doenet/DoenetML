import { u as le, d as ce, e as ie, C as H, g as I, j as F, t as ae, M as se } from "./index-dce15b2c.js";
import ue, { useRef as l, useContext as de, useEffect as W } from "react";
import { BoardContext as fe, TEXT_LAYER_OFFSET as q } from "./graph-b13204d4.js";
const ye = ue.memo(function K(Q) {
  let { name: S, id: D, SVs: r, children: he, actions: u, callAction: d } = le(Q);
  K.ignoreActionsWithoutCore = () => !0;
  let e = l(null), _ = l(null), A = l(null);
  const t = de(fe);
  let y = l(null), P = l(null), L = l(!1), X = l(!1), s = l(!1), f = l(null), h = l(null), B = l(null), j = l(null), b = l(!1), v = l(!1);
  ...
  function Z() {
    let i = C === "dark" ? r.selectedStyle.textColorDarkMode : r.selectedStyle.textColor, a = C === "dark" ? r.selectedStyle.backgroundColorDarkMode : r.selectedStyle.backgroundColor, m = "";
    a && (m += `background-color: ${a}`);
    let p = {
      visible: !r.hidden,
      fixed: b.current,
      layer: 10 * r.layer + q,
    ...
    try {
      let n = H.fromAst(r.anchor), g = [
        n.get_component(0).evaluate_to_constant(),
        n.get_component(1).evaluate_to_constant()
      ];
      Number.isFinite(g[0]) || (g[0] = 0, p.visible = !1), Number.isFinite(g[1]) || (g[1] = 0, p.visible = !1), c = t.create("point", g, {
        visible: !1
      });
    } catch {
    ...
    o.isDraggable = !v.current, o.on("down", function(n) {
      y.current = [n.x, n.y], P.current = [...c.coords.scrCoords], s.current = !1, L.current = !0, X.current = !1, b.current || d({
      ...
      let g = n.type === "pointermove";
      (!g || Math.abs(n.x - y.current[0]) > 0.1 || Math.abs(n.y - y.current[1]) > 0.1) && (s.current = !0);
      ...
      if (g) {
        ...
        f.current = (P.current[1] + n.x - y.current[0] - $[1]) / t.unitX, h.current = ($[2] - (P.current[2] + n.y - y.current[1])) / t.unitY;
      } else
  ...
  function N(i) {
    L.current && (Math.abs(i.x - y.current[0]) > 0.1 || Math.abs(i.y - y.current[1]) > 0.1) && (X.current = !0);
  }
  function ee() {
